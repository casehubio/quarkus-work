...
**Inbound work item creation (engine#974):** `InboundWorkItemSchedulerImpl` implements `InboundWorkItemScheduler` SPI (engine-common). Converts `InboundWorkItemRequest` to `WorkItemCreateRequest`, wraps creation in `TenantContextRunner.runInTenantContext()` (request arrives outside request scope from qhorus afterCompletion callback). Overrides engine's `NoOpInboundWorkItemScheduler` (`@DefaultBean`) when engine-adapter is on the classpath. Priority mapping: String → `WorkItemPriority.valueOf()`, null-safe, fail-fast on invalid values. Cross-repo consumers (engine `InboundWorkItemBridge`) use `WorkItemOperations.createInTenantContext()` instead (#399).
